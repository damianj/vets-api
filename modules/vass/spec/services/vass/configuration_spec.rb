...
RSpec.describe Vass::Configuration do
  ...
  before do
    configuration.send(:instance_variable_set, :@__oauth_conn_pool__, nil)
  end
  ...
  describe '#oauth_connection' do
    it 'uses Settings.vass.oauth_open_timeout and oauth_read_timeout on the Faraday connection' do
      ...
      conn = configuration.oauth_connection(server_url: oauth_url)
    ...
    it 'reflects updated Settings when a new connection is built' do
      ...
      first = configuration.oauth_connection(server_url: oauth_url)
      ...
      second = configuration.oauth_connection(server_url: oauth_url)
