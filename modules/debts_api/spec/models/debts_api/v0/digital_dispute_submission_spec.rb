...
RSpec.describe DebtsApi::V0::DigitalDisputeSubmission do
  ...
  describe 'Lockbox encryption' do
    it 'defines LOCKBOX with encrypt/decrypt round-trip' do
      lockbox = described_class::LOCKBOX
      expect(lockbox).to respond_to(:encrypt, :decrypt)
      plain = 'secret-pii@example.com'
      encrypted = lockbox.encrypt(plain)
      expect(encrypted).not_to eq(plain)
      expect(lockbox.decrypt(encrypted)).to eq(plain)
    end
  end
  ...
  describe 'email functionality' do
    ...
    describe '#send_failure_email' do
      ...
      it 'enqueues failure email when user is found with Lockbox-encrypted user_pii and no cache' do
        ...
        ) do |_delay, identifier, _template_id, personalisation, _options|
          expect(described_class::LOCKBOX.decrypt(identifier)).to eq('test@example.com')
          expect(described_class::LOCKBOX.decrypt(personalisation['first_name'])).to eq('John')
        end
    ...
    describe '#send_success_email' do
      ...
      it 'encrypts user_pii with Lockbox' do
        expect(DebtsApi::V0::Form5655::SendConfirmationEmailJob).to receive(:perform_async) do |args|
          expect(described_class::LOCKBOX.decrypt(args['user_pii'][:email])).to eq('test@example.com')
          expect(described_class::LOCKBOX.decrypt(args['user_pii'][:first_name])).to eq('John')
        end
        form_submission.send(:send_success_email)
      end
    end
