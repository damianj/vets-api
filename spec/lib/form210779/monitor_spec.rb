...
RSpec.describe Form210779::Monitor do
    ...
    context 'with ActiveRecord validation error' do
      ...
      it 'logs validation error with field path' do
        expect(Rails.logger).to receive(:warn).with(
          "form210779:#{form_id} validation failed: activerecord_validation",
          hash_including(
  ...
  describe '#track_submission_success' do
    ...
    it 'increments StatsD metrics for both submission and request' do
      expect(StatsD).to receive(:increment).with(
    ...
    it 'logs at info level twice with claim context' do
  ...
  describe '#track_submission_failure' do
    ...
    it 'logs error and info messages' do
  ...
  describe '#track_pdf_generation_success' do
    let(:start_time) { 2.seconds.ago }
    let(:user_uuid) { 'test-user-uuid-pdf' }
    let(:claim_guid) { SecureRandom.uuid }

    it 'measures PDF generation duration' do
      expect(StatsD).to receive(:measure).with(
        "#{claim_stats_key}.pdf_generation.duration",
        instance_of(Float)
      )
      ...
      monitor.track_pdf_generation_success(start_time, user_uuid:, claim_guid:)
    end

    it 'increments StatsD metrics for both generation and request' do
    ...
    it 'logs at info level twice with duration' do
      allow(StatsD).to receive(:measure)
      allow(StatsD).to receive(:increment)
  ...
  describe '#track_pdf_generation_failure' do
    let(:error) { StandardError.new('PDF generation error') }
    let(:user_uuid) { 'test-user-uuid-pdf-failure' }
    let(:claim_guid) { SecureRandom.uuid }
    ...
    it 'logs error and info messages with error details' do
      expect(Rails.logger).to receive(:error).with(
        anything,
        hash_including(
          context: hash_including(
            error_class: 'StandardError',
            error_message: 'PDF generation error'
          )
        )
      )
      expect(Rails.logger).to receive(:info)
    ...
    it 'infers correct status code for different error types' do
      allow(Rails.logger).to receive(:error)
      allow(Rails.logger).to receive(:info)
      allow(StatsD).to receive(:increment)

      not_found_error = ActiveRecord::RecordNotFound.new
      monitor.track_pdf_generation_failure(not_found_error, claim_guid:)

      expect(Rails.logger).to have_received(:info).with(
        anything,
        hash_including(context: hash_including(code: 404))
      )
    end
  end
end
