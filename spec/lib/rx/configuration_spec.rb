...
RSpec.describe Rx::Configuration do
  ...
  describe '#base_path' do
    it 'returns the API gateway base path' do
      ...
      expect(service_config.base_path).to eq('https://api-gateway.example.com/v1/')
    end
  end
  ...
  describe '#caching_enabled?' do
    it 'returns true if collection caching is enabled' do
      ...
      expect(service_config.caching_enabled?).to be true
    end
    ...
    it 'returns false if collection caching is not enabled' do
      ...
      expect(service_config.caching_enabled?).to be false
    end
  end

  describe '#service_name' do
    it 'returns the service name' do
      expect(service_config.service_name).to eq('Rx')
    end
  end

  describe '#breakers_error_threshold' do
    it 'returns the error threshold for breakers' do
      expect(service_config.breakers_error_threshold).to eq(80)
    end
  end
end
