...
RSpec.describe Form21p530a::Monitor do
  ...
  describe '#track_submission_begun' do
    ...
    it 'increments StatsD metric with tags' do
      expect(StatsD).to receive(:increment).with(
        ...
        hash_including(tags: array_including('service:form21p530a', 'action:create', 'status:begun'))
      )
  ...
  describe '#track_submission_success' do
    ...
    it 'increments StatsD metrics with tags' do
      expect(StatsD).to receive(:increment).with(
        ...
        hash_including(tags: array_including('service:form21p530a', 'action:create', 'status:success'))
      )
      expect(StatsD).to receive(:increment).with(
        "#{claim_stats_key}.request",
        hash_including(tags: array_including('service:form21p530a', 'status_code:200', 'action:create'))
      )
  ...
  describe '#track_submission_failure' do
    ...
    it 'increments StatsD metrics with tags' do
      ...
      expect(StatsD).to receive(:increment).with(
        "#{claim_stats_key}.request",
        hash_including(tags: array_including('service:form21p530a', 'status_code:500', 'action:create'))
      )
  ...
  describe '#track_request_code' do
    it 'increments StatsD metric with dynamic tags' do
      expect(StatsD).to receive(:increment).with(
        "#{claim_stats_key}.request",
    ...
    context 'with pattern mismatch error' do
      ...
      it 'logs validation error with warn level' do
        expect(Rails.logger).to receive(:warn).with(
          "form21p530a:#{form_id} Committee validation failed",
          hash_including(
